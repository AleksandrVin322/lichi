...
import '../../client/database_client.dart';
...
/// [Bloc] для корзины товаров.
class CartBloc extends Bloc<CartEvent, CartState> {
  final DatabaseClient databaseClient;

  CartBloc({required this.databaseClient}) : super(CartInitialState()) {
  ...
  void _initCart(InitialEvent event, Emitter<CartState> emit) async {
    try {
      final Cart cart = await databaseClient.loadCart();
  ...
  void _addProduct(AddProductEvent event, Emitter<CartState> emit) async {
    try {
      ...
      final products = {...currentState.cart.products};
  ...
  /// Увеличение количества продукта на 1 в корзине.
  ...
  /// Уменьшение количества продукта на 1 в корзине.
...
/// Расчет суммы корзины и количества продуктов в корзине.
